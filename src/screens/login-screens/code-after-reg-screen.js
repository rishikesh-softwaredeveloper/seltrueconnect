...
const CodeAfterRegScreen = ({ route }) => {
  ...
  const verifyOtp = () => {
    ...
    PostVerifyOTP(checkotp).then((Res)=>{
      if(Res.status ==1){
        navigation.navigate('GeneratePinAfterRegScreen', {registerParams,mobile_no})
      }else{
        alert(Res.message)
      }
    })
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={styles.buttonsContainer}></View>
        <Text style={{ fontSize: 40, fontWeight: 'bold', marginVertical: 20, marginHorizontal: 20,fontFamily:'serif' }}>4-digit code{"\n"}
        <Text style={{ fontSize: 15, marginVertical: 20, marginHorizontal: 20,fontFamily:'serif' }}>Please enter the code we've sent to {mobile_no} </Text></Text>
...
export default CodeAfterRegScreen;
