...
import SalesDealerItem from "../../components/salesDealer-items";
import { GetVendorList } from "../../services/bundle-services/get-vendor-list";
import MyDealerItem from "../../components/myDealer-items";

const { width,height } = Dimensions.get("screen");

const MyDealersScreen = () => {
  ...
  useEffect(()=>{
    ...
    GetVendorList(token,dealer[0].vendor_id).then((resSales)=>{
    ...
    return( 
      <View style={{flex:1,margin:20}}>
        <FlatList
          ...
          renderItem={({ item }) => (<MyDealerItem VendorListItem={item} />)}
