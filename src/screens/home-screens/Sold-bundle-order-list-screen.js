...
import { GetPurchaseBundleItemList } from '../../services/bundle-services/get-purchase-bundle-item-list';
import PurchaseBundleItemsList from '../../components/purchase-bundle-items-list';
import SoldBundleItemsList from '../../components/sold-bundle-items-list';

const SoldBundleOrderListScreen = ({ route }) => {
  ...
  useEffect(()=>{
    ...
    const reqData ={
      ...
      "order_item_id" :bundleItem.bundleItem['order_item_id']
    }

    GetPurchaseBundleItemList(reqData,token).then((Res)=>{
      if(Res?.status == 1){
        setData(Res?.results);
        setSpinner(false);
        return false;
      ...
      </View>
      {
        spinner ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => <SoldBundleItemsList bundleItem={{...item,"bundleStatus":bundleItem.bundleItem['bundle_status']}} />}
...
export default SoldBundleOrderListScreen;