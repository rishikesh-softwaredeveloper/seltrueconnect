...
import { PostUpdateOrderStatus } from "../services/bundle-services/post-update-order-status";
import { GetPurchasedOrders } from "../services/bundle-services/get-purchased-orders";

const { width,height } = Dimensions.get("screen");

const PurchaseOrderItem = (orderItem) => {
    const [selectedPrinter, setSelectedPrinter] = useState();
    const types = ["Invoice", "Items"];
    const token = useSelector((state) => state.token);
    const [isVisible, setisVisible] = useState(false);
    const OrderStatusTypes = ["PENDING", "APPROVED"];
    ...
    const OrderDetails=(Item)=>{
        navigation.navigate('PurchaseOrderListScreen', {"orderItems":Item})    
    }
                    ...
                    <View style={styles.sub_container}>
                        <View style={{ borderWidth: 0, borderColor: "#CCC", margin: 0, padding: 5,paddingBottom: 0,paddingLeft:15, borderRadius: 0, width: '50%' }}>
                            <View style={{ paddingBottom: 0 }}>
                                <View style={{ paddingBottom: 15, flexDirection: 'row', justifyContent: 'space-between',marginLeft:10 }}>
                                    <Text style={{ fontWeight: '600', color: "#000",fontFamily:'serif',fontSize:18 }}>{orderItem[keys]?.order_no.toUpperCase()}</Text>
                                </View>
                                <Image  source={require('../../assets/seltrue-1.png')} style={{width:80,height:30,marginBottom:10}}/>

                                <View style={{ paddingBottom: 0, flexDirection: 'row', justifyContent: 'space-between',marginLeft:10 }}>
                                    <Text style={{ fontWeight: '600', color: "#000",fontFamily:'serif',fontSize:15 }}>{orderItem[keys]?.order_placed_on}</Text>
                                </View>

                            </View>
                        </View>
                        
                        <View style={{  borderWidth: 0, borderColor: "#CCC", margin: 0, padding: 5, paddingBottom: 0,borderRadius: 0, width: '40%',marginRight: "5%",marginTop:0  }}>
                            <View style={{flexDirection:'column',justifyContent:'flex-end'}}>
                                <View style={{ paddingBottom: 15, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={{ fontWeight: '500', color: "#000",fontFamily:'serif',fontSize:18 }}>&#8377;{parseFloat(orderItem?.[keys]?.purchase_price).toFixed(2)?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}/-</Text>
                                </View>
                                {dealer[0].category_id != 4 ?
                                 <TouchableOpacity style={{ paddingBottom: 15, flexDirection: 'row', justifyContent:'space-between' }} onPress={()=>{
                                    dealer[0].category_id != 4 &&
                                    setisVisible(true)
                                   }}>
                                       <Text style={{ color:(orderItem[keys].order_status == 'INVOICED')?"#3d9446":"#DD2A05",fontSize:15,fontFamily:'serif',fontWeight:'500' }}>{orderItem[keys].order_status}</Text>
                                   </TouchableOpacity>
                                   :
                                   <View style={{ paddingBottom: 15, flexDirection: 'row', justifyContent:'space-between' }} >
                                       <Text style={{ color:(orderItem[keys].order_status == 'INVOICED')?"#3d9446":"#DD2A05",fontSize:15,fontFamily:'serif',fontWeight:'500' }}>{orderItem[keys].order_status}</Text>
                        ...
                        <View style={{flexDirection:'row',justifyContent:'flex-start',width: '10%',alignItems:'center'}}>
                            {
                                (orderItem[keys].order_status == 'INVOICED')?(
                                    <SelectDropdown
                                    data={types}
                                    onSelect={(selectedItem, index) => {
                                        if(selectedItem == "Invoice"){
                                            print(orderItem[keys].order_no)
                                        }
                                        if(selectedItem=="Items"){
                                            print2(orderItem[keys].order_no)
                                        }
                        ...
                        <View style={{ margin: 0, padding: 0, borderStartWidth: 0, borderColor: "#CCC", justifyContent: 'center', width: "20%" }}>
                            <View style={{ alignItems: 'center', justifyContent: 'center'}}>
                                <IconButton
                                    ...
                                    size={30}
                                    color="gray"
                                    onPress={()=>OrderDetails(orderItem[keys]?.order_id)}
                                />
                            </View>
                        </View>
                        <Modal
                        ...
                        <View style={{ backgroundColor:'#ffffff', width:width-60,borderRadius:5}}>
                            ...
                            <View style={{padding:40,width:"90%"}}>
                                <View style={{marginBottom:30,flexDirection:'row',}}>
                                    <Text style={{color:'#000',fontWeight:'bold',width:'50%'}}>Order Status : </Text>
                                    <Text style={{color:'#000',fontWeight:'bold'}}>{orderItem?.orderItem?.order_status}</Text>
                                </View>
                                {
                                    //stss and dist
                                    dealer[0].category_id == 2 ||  dealer[0].category_id == 3  ?
                                    orderItem?.orderItem?.available_bundles != 0 ?
                                    <View style={{flexDirection:'row',marginBottom:20,alignItems:'center'}}>
                                        <Text style={{color:'#000',fontWeight:'bold',width:'50%'}}>Update Status :</Text>
                                        <View style={{}}>
                                            <SelectDropdown
                                                ...
                                                onSelect={(selectedItem, index) => {
                                                    ...
                                                    PostUpdateOrderStatus(Reqdata,token).then((Res)=>{
                                                ...
                                                defaultButtonText={orderItem?.orderItem?.order_status}
                                                buttonTextAfterSelection={(selectedItem, index) => {
                                                    return selectedItem;
                                                }}
                                                rowTextForSelection={(item, index) => {
                                                ...
                                                rowTextStyle ={{fontSize:10,fontFamily:'serif'}}
                                                buttonStyle={{borderRadius:0,height:35,width:140,backgroundColor:"#fff",borderWidth:1}}
                                            />
                                        </View>
                                    ...
                                    :<></>
                                    :<View>
                                        <Text style={{color:'red',fontWeight:'bold'}}>ALL BUNDLES ARE IN SOLD</Text>
                                    </View>
                                }
                            </View>
