...
const SoldOrderItem = (orderItem) => {
    ...
    const OrderDetails=(Item)=>{
        navigation.navigate('SoldOrderListScreen', {"orderItems":Item})    
    }
    
    for (var keys in orderItem) {
        return (
            <View style={styles.container}>
                <TouchableOpacity onPress={()=>OrderDetails(orderItem[keys]?.order_id)}>
                    <View style={styles.sub_container}>
                        <View style={{ borderWidth: 0, borderColor: "#CCC", margin: 0, padding: 5,paddingBottom: 0,paddingLeft:15, borderRadius: 0, width: '60%' }}>
                            <View style={{ paddingBottom: 0 }}>
                                <TouchableOpacity style={{ paddingBottom: 15, flexDirection:'row',alignItems:'center' }} 
                                // onPress={()=>{(dealer[0].category_id == 2 || dealer[0].category_id == 3 )? setisVisible(true) : null}}
                                >
                                    <Text style={{ fontWeight: '600', color: "#000",fontFamily:'serif',fontSize:18 }}>{orderItem[keys]?.order_no.toUpperCase()}</Text>
                                    <Text style={{ fontWeight: '600', color: "#000",fontSize:12,fontFamily:'serif',marginLeft:10 }}>{"("+orderItem[keys].order_status+")"}</Text>
                                </TouchableOpacity>
                                ...
                                <Text style={{ fontWeight: '600', color: "#000",fontFamily:'serif',fontSize:15 }}>{orderItem[keys]?.vendor_name}</Text>
                        ...
                        <View style={{  borderWidth: 0, borderColor: "#CCC", margin: 0, padding: 5, paddingBottom: 0,borderRadius: 0, width: '30%',marginRight: "5%",marginTop:0  }}>
                            <View style={{flexDirection:'column',justifyContent:'flex-end'}}>
                                <View style={{ paddingBottom: 15, flexDirection: 'row', justifyContent: 'space-between' }}>
                                    <Text style={{ fontWeight: '500', color: "#000",fontFamily:'serif',fontSize:16 }}>&#8377;{ parseFloat(orderItem?.[keys]?.sale_price).toFixed(2)?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}/-</Text>
                                </View>
                                <View style={{ paddingBottom: 15, flexDirection: 'row', justifyContent:'space-between' }}>
                                    <Text style={{ color:(orderItem[keys].order_status == 'INVOICED')?"#3d9446":"#DD2A05",fontSize:15,fontFamily:'serif',fontWeight:'500' }}>Bundle Qty : {orderItem[keys].bundles_quantity}</Text>
                                </View>
                            </View>
                        </View>
                        <Modal
                                        ...
                                        <View style={{}}>
                                            <SelectDropdown
                                                ...
                                                onSelect={(selectedItem, index) => {
                                                    ...
                                                    // const Reqdata={
                                                    //     // "bundle_id" :orderItem?.orderItem?.bundle_id,
